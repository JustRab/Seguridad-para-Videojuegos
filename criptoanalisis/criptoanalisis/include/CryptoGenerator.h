...
public:
    ...
    return oss.str();  // Devuelve la representación hexadecimal como cadena.
	}
  ...
  std::vector<uint8_t>
    ...
    return data;  // Devuelve el vector de bytes decodificados.
	}

	/**
   * @brief Genera una clave simétrica de tamaño dado en bits.
	 * 
   * @param bits: Tamaño de la clave en bits (Debe ser multiplo de 8).
   * @return std::vector<uint8_t> Vector con la clave generada.
   * @throws std::runtime_error Si el tamaño de bits no es múltiplo de 8.
   */

  std::vector<uint8_t>
  generateKey(unsigned int bits) {
    if (bits % 8 != 0) {
      throw std::runtime_error("Key size must be a multiple of 8 bits.");
    }
    return generateBytes(bits / 8);  // Genera la clave como un vector de bytes.
  }

	/**
    *@brief Genera un vector de inicialización (IV) de tamaño dado en bytes.
		* Un IV es un valor aleatorio que se usa en modos de cifrado simétrico (CBC, CFB, GCM_)
		* para garantizar que la misma clave cifre mensajes idénticos en salidas distintas
		* 
    * @param blockSize: Tamaño del IV en bytes.
    * @return std::vector<uint8_t> Vector con el IV generado.
		*/
  std::vector<uint8_t>
   generateIV(unsigned int blockSize) {
    return generateBytes(blockSize);  // Genera un IV como un vector de bytes.
  }

	/**
   * @brief Genera una salta criptográfica de longitud dada en bytes.
	 * Una salt es un valor aleatorio que se combina con la contraseña al derivar una clave
   * (por ejemplo, en PBKDF2, scrypt, Argon2) para evitar ataques de tabla arcoiris y aegurar
	 * que cada derivación sea única. Salt se usa para "esperar" la entropía de contraseñas
	 * al dderivar claves
	 * 
   * @param length: Longitud de la salt en bytes.
   * @return std::vector<uint8_t> Vector con la salt generada.
   */
  std::vector<uint8_t>
	generateSalt(unsigned int length) {
		return generateBytes(length);  // Genera una salt como un vector de bytes.
	}

	/**
   * @brief Convierte un vector en bytes a una cadena Base64.
	 * 
   * @param data: Vector de bytes de entrada.
   * @return std::string Cadena Base64 resultante. (Con relleno '=' si es necesario).
	 */
  std::string
    ...
    std::string b64;
    unsigned int i = 0;
    // Procesar los bytes de 3 en 3 para codificar en Base64.
		// Tomamos 3 bytes y los cocatenamos en un entero de 24 bits.
    // Ese entero lo dividimos en cuatro "seis bits" (24 / 6 = 4).
		// Cada grupo de 6 bits (valor 0-63) se usa como indice en la table para obtener un carácter.
		while (i + 2 < data.size()) {
      unsigned int block = (data[i] << 16) | (data[i + 1] << 8) | data[i + 2];
      b64 += base64Chars[(block >> 18) & 0x3F];
      b64 += base64Chars[(block >> 12) & 0x3F];
      b64 += base64Chars[(block >> 6) & 0x3F];
      b64 += base64Chars[block & 0x3F];
      i += 3;  // Procesa bloques de 3 bytes a la vez.
		}

    // Procesar los ultimos 1 o 2 bytes restantes, añadiendo relleno ('=') si es necesario.
    if (i < data.size()) {
      ...
      if (i + 1 < data.size()) {
        ...
        b64 += base64Chars[(block >> 12) & 0x3F];
        b64 += "=="; // Añade dos signos de igual para el relleno.
      }
    }

    return b64;  // Devuelve la cadena Base64 resultante.
	}
	...
	passwordStrength(const std::string& password) {
		...
		if (entropy < 28) {
			...
			return "Fuerte";  // Entropía entre 60 y 80 bits.c
		}
		else {
